...
Open `http://YOUR_VPS_IP:3000`. The Compose volume `cloud-torrent-data` persists downloads, engine databases, settings, and the torrent library across container restarts. Stop it with `docker compose down`; start it again with `docker compose up -d`.
...
Use the `Settings` link in the top bar or open http://localhost:3000/settings.html to configure the server download path, peer uploads, and completed-torrent seeding. These engine settings are applied after restarting the server.
